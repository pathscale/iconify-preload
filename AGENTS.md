# Working agreement — iconify-preload
...
**JavaScript/TypeScript library** (`@pathscale/rsbuild-plugin-iconify`), built with `bun`.
...
**Always paste the full PR URL** (`https://github.com/pathscale/iconify-preload/pull/<n>`), not just the number, so it's
clickable.
...
uncomment the rule below.
    ...
    ```bash
    gh pr view <number> --repo pathscale/iconify-preload --json statusCheckRollup
    ```
